  ...
  def udp_recv(self, sock):
    ...
    try:
      ...
      if ret:
        ...
        if 'cmd' in json_obj:
          # SECURITY: the upstream implementation ran os.system(json_obj['cmd'])
          # here, i.e. it executed arbitrary shell commands sent over an
          # unauthenticated UDP socket. Anyone who could reach this port (e.g.
          # another device on the same WiFi network) could run arbitrary code
          # on the driving computer. Intentionally removed for opkr-op3t.
          ret = False
